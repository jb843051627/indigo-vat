...
func TestBug17_PublishHonorsCancellation(t *testing.T) {
    _, svc := testDB(t)
    recipe, err := svc.CreateRecipe(context.Background(), model.RecipeInput{Name: "cancelled", TargetHue: 220, HueTolerance: 5, MinMinutes: 1, MaxMinutes: 2})
    if err != nil { t.Fatal(err) }
    if _, err := svc.AddStage(context.Background(), recipe.ID, model.StageInput{Name: "ferment", Minutes: 1, PHMin: 8, PHMax: 12, TempMin: 20, TempMax: 40}); err != nil { t.Fatal(err) }
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    if _, err := svc.PublishRecipe(ctx, recipe.ID); !errors.Is(err, context.Canceled) { t.Fatalf("want cancellation, got %v", err) }
}
