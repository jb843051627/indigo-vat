...
func TestBug08_StaleRevisionCannotAdvanceCycle(t *testing.T) {
    _, svc := testDB(t)
    cycle := seedCycle(t, svc)
    if _, err := svc.AdvanceCycle(context.Background(), cycle.ID, model.CycleMatured, 1, "ready"); err != nil { t.Fatal(err) }
    if _, err := svc.AdvanceCycle(context.Background(), cycle.ID, model.CycleHeld, 1, "stale"); !errors.Is(err, service.ErrConflict) { t.Fatalf("want conflict, got %v", err) }
}
