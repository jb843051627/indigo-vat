...
func TestBug27_AcceptSamplePropagatesClosedDatabaseError(t *testing.T) {
    db, svc := testDB(t)
    cycle := seedCycle(t, svc)
    if _, err := svc.RecordSample(context.Background(), model.SampleInput{CycleID: cycle.ID, Hue: 220, PH: 10, Temperature: 30, Observer: "operator"}); err != nil { t.Fatal(err) }
    samples, err := db.ListSamples(context.Background(), cycle.ID); if err != nil || len(samples) != 1 { t.Fatalf("seed sample failed: %v", err) }
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    if _, err := svc.AcceptSample(ctx, samples[0].ID); !errors.Is(err, context.Canceled) { t.Fatalf("cancellation was swallowed: %v", err) }
}
