...
func TestBug11_RetryStopsWhenCancelled(t *testing.T) {
    ctx, cancel := context.WithCancel(context.Background())
    calls := 0
    start := time.Now()
    err := worker.Retry(ctx, 3, 200*time.Millisecond, func(context.Context) error { calls++; if calls == 1 { cancel() }; return errors.New("temporary") })
    if !errors.Is(err, context.Canceled) { t.Fatalf("want cancellation, got %v", err) }
    if time.Since(start) > 100*time.Millisecond { t.Fatal("retry ignored cancellation during backoff") }
}
