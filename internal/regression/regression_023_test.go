...
func TestBug23_CreateCycleConflictReturns409(t *testing.T) {
	_, svc := testDB(t)
	vat, err := svc.CreateVat(context.Background(), model.VatInput{Name: "Vat", Site: "Shed", Capacity: 100, TargetPH: 10, TargetTemp: 30, Timezone: "UTC"})
	if err != nil { t.Fatal(err) }
	if _, err := svc.PauseVat(context.Background(), vat.ID); err != nil { t.Fatal(err) }
	recipe, err := svc.CreateRecipe(context.Background(), model.RecipeInput{Name: "Recipe", TargetHue: 220, HueTolerance: 5, MinMinutes: 1, MaxMinutes: 2})
	if err != nil { t.Fatal(err) }
	if _, err := svc.AddStage(context.Background(), recipe.ID, model.StageInput{Name: "Ferment", Minutes: 1, PHMin: 8, PHMax: 12, TempMin: 20, TempMax: 40}); err != nil { t.Fatal(err) }
	if _, err := svc.PublishRecipe(context.Background(), recipe.ID); err != nil { t.Fatal(err) }
	handler := httpapi.New(svc).Handler()
	body := strings.NewReader(`{"vat_id":"` + vat.ID + `","recipe_id":"` + recipe.ID + `"}`)
	request := httptest.NewRequest(http.MethodPost, "/api/v1/cycles", body)
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusConflict { t.Fatalf("want 409, got %d", response.Code) }
}
