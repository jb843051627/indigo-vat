...
func TestBug16_RecipeStagesAreCopied(t *testing.T) {
    original := model.Recipe{Stages: []model.RecipeStage{{ID: "stage-1", Name: "ferment"}}}
    clone := model.CloneRecipe(original)
    clone.Stages[0].Name = "changed"
    if original.Stages[0].Name != "ferment" { t.Fatal("recipe stages were aliased") }
}
