...
func TestBug26_SampleQueryPropagatesCancellation(t *testing.T) {
    db, _ := testDB(t)
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    if _, err := db.ListSamples(ctx, "cycle-sample"); !errors.Is(err, context.Canceled) { t.Fatalf("want cancellation, got %v", err) }
}
