...
func TestBug04_ReportHonorsCancellation(t *testing.T) {
    db, _ := testDB(t)
    cycle := model.Cycle{ID: "cycle-cancel", State: model.CycleMatured, Revision: 1}
    if err := db.PutCycle(context.Background(), cycle); err != nil { t.Fatal(err) }
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    _, err := db.Report(ctx, cycle.ID)
