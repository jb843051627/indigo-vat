...
func TestBug02_SampleForMissingCycleReturnsError(t *testing.T) {
    _, svc := testDB(t)
    _, err := svc.RecordSample(context.Background(), model.SampleInput{CycleID: "missing-cycle", Hue: 220, PH: 10, Temperature: 30, Observer: "operator"})
    if !errors.Is(err, service.ErrNotFound) { t.Fatalf("want missing cycle error, got %v", err) }
}
