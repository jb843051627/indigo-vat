...
func TestBug30_ReportCloneDoesNotShareAlerts(t *testing.T) {
    original := model.ReleaseReport{Alerts: []model.Alert{{ID: "alert-1", State: model.AlertOpen}}}
    cloned := model.CloneReport(original)
    cloned.Alerts[0].State = model.AlertAcknowledged
    if original.Alerts[0].State != model.AlertOpen { t.Fatal("release alerts were aliased") }
}
