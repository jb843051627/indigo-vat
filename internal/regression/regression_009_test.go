...
func TestBug09_ReleasedCycleCannotResume(t *testing.T) {
    _, svc := testDB(t)
    cycle := seedCycle(t, svc)
    matured, err := svc.AdvanceCycle(context.Background(), cycle.ID, model.CycleMatured, 1, "matured")
    if err != nil { t.Fatal(err) }
    released, err := svc.AdvanceCycle(context.Background(), matured.ID, model.CycleReleased, matured.Revision, "released")
    if err != nil { t.Fatal(err) }
    if _, err := svc.ResumeCycle(context.Background(), released.ID, released.Revision, "resume"); err == nil { t.Fatal("released cycle resumed") }
}
