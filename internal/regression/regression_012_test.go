...
func TestBug12_MissingReportIsNotSuccess(t *testing.T) {
    _, svc := testDB(t)
    handler := httpapi.New(svc).Handler()
    request := httptest.NewRequest(http.MethodGet, "/api/v1/cycles/missing-cycle/report", nil)
    response := httptest.NewRecorder()
    handler.ServeHTTP(response, request)
    if response.Code != http.StatusNotFound { t.Fatalf("want 404, got %d", response.Code) }
}
