...
func TestBug03_ReportCloneDoesNotShareSamples(t *testing.T) {
    original := model.ReleaseReport{Samples: []model.Sample{{ID: "sample-1", Hue: 220}}}
    cloned := model.CloneReport(original)
    cloned.Samples[0].Hue = 100
    if original.Samples[0].Hue != 220 { t.Fatal("release dossier samples were aliased") }
}
