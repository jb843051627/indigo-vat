...
func TestBug22_MissingVatStateChangeReturnsError(t *testing.T) {
    db, _ := testDB(t)
    err := db.SetVatState(context.Background(), "missing-vat", model.VatPaused, time.Now().Format(time.RFC3339Nano))
    if !errors.Is(err, sql.ErrNoRows) { t.Fatalf("want sql.ErrNoRows, got %v", err) }
}
