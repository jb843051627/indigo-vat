...
func TestBug29_HueDistanceWrapsAtTheColorBoundary(t *testing.T) {
    err := validation.ValidateSample(model.SampleInput{CycleID: "cycle-1", Hue: 220, PH: 20, Temperature: 30, Observer: "operator"})
    if !errors.Is(err, validation.ErrInvalidReading) { t.Fatalf("want ErrInvalidReading, got %v", err) }
}
