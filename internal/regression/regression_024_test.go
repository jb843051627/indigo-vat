...
func TestBug24_MetricResetDoesNotRaceWithAdd(t *testing.T) {
    registry := metrics.New()
    var wg sync.WaitGroup
    for i := 0; i < 6; i++ { wg.Add(1); go func() { defer wg.Done(); for j := 0; j < 400; j++ { registry.Add("inspections", 1); _ = registry.Snapshot() } }() }
    wg.Wait()
}
