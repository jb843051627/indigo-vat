...
func TestBug21_AlertCountPropagatesCancellation(t *testing.T) {
    db, _ := testDB(t)
    ctx, cancel := context.WithCancel(context.Background()); cancel()
    if _, err := db.OpenAlertCount(ctx, "cycle-alert"); !errors.Is(err, context.Canceled) { t.Fatalf("want cancellation, got %v", err) }
}
