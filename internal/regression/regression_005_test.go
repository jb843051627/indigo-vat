...
func TestBug05_MetricsSnapshotIsSafeDuringUpdates(t *testing.T) {
    registry := metrics.New()
    var wg sync.WaitGroup
    for i := 0; i < 8; i++ { wg.Add(1); go func() { defer wg.Done(); for j := 0; j < 500; j++ { registry.Inc("samples"); _ = registry.Snapshot() } }() }
    wg.Wait()
}
