...
func TestBug14_MissingInspectionReturnsNotFound(t *testing.T) {
    _, svc := testDB(t)
    _, err := svc.CompleteInspection(context.Background(), "missing-inspection")
    if !errors.Is(err, service.ErrNotFound) { t.Fatalf("want not found, got %v", err) }
}
