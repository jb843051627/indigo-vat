...
func (d *DB) ReleaseAtomic(ctx context.Context, id string, revision int, when string, e model.AuditEvent) error {
	result, err := d.Exec(ctx, `UPDATE cycles SET state=?,revision=revision+1,released_at=?,updated_at=? WHERE id=? AND revision=?`, model.CycleReleased, when, when, id, revision)
	if err != nil {
		return err
	}
