...
func (d *DB) CreateCycleAtomic(ctx context.Context, c model.Cycle, e model.AuditEvent) error {
	if _, err := d.Exec(ctx, `INSERT INTO cycles(id,vat_id,recipe_id,state,revision,started_at,matured_at,released_at,updated_at,note) VALUES(?,?,?,?,?,?,?,?,?,?)`, c.ID, c.VatID, c.RecipeID, c.State, c.Revision, Text(c.StartedAt), Text(c.MaturedAt), Text(c.ReleasedAt), Text(c.UpdatedAt), c.Note); err != nil {
		return err
	}
...
func (d *DB) CreateSampleAtomic(ctx context.Context, s model.Sample, e model.AuditEvent) error {
	return d.WithTx(ctx, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `INSERT INTO samples(id,cycle_id,taken_at,hue,ph,temperature,status,observer,note) VALUES(?,?,?,?,?,?,?,?,?)`, s.ID, s.CycleID, Text(s.TakenAt), s.Hue, s.PH, s.Temperature, s.Status, s.Observer, s.Note); err != nil {
			return err
		}
